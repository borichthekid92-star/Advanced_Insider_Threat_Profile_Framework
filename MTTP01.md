...
Built-in OS utilities 9. del /f /q (Windows CMD) — force-quiet deletion, bypasses recycle bin 10. rm -rf (Linux/macOS) — recursive force delete, no recovery prompt 11. shred (Linux) — overwrites file data before unlinking, defeats simple carving 12. wipe (Linux) — secure unlink with multiple overwrite passes
